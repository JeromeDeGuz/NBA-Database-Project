...
class MyDatabase {
    ...
    public void leagueAvg(String stat, String season, String limit) {
        try {
            ...
            if (check) {
                sql = "select p.firstname,p.lastName, avg(gps." + stat
                        + ") as avg from GamePlayerStats gps join Player p on gps.playerID = p.playerID join RegularGame rg on gps.gameID = rg.gameID where rg.seasonYear = ? group by p.firstname,p.lastName, order by avg desc limit ?;";
            } else {
